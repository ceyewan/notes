...
- 从根节点到节点 $N$ 的路径字符串定义为从根到节点 $N$ 的所有边上的字符串的连接。
...
1. **ind(N)**：表示从根节点到节点 $N$ 的路径字符串在原始字符串中的首次出现位置。
   - $ind(N_{12}) = (2, 3)$，因为 "lp" 在第二个字符串的第 3 个位置首次出现。
2. **path(N)**：表示从根节点到节点 $N$ 的完整路径字符串。
   - $path(N_7) = "hello"$。
3. **initpath(N)**：表示从根节点到节点 $N$ 的路径字符串，但最后一条边仅取首字符。
   - $initpath(N_7) = "hell"$，因为路径到 $N_7$ 的最后一条边是 "o"，只取 "o" 的首字符。
4. **leafpos(N)**：表示以节点 $N$ 为根的子树中最左侧叶节点的位置。
   - $leafpos(N_3) = 3$。
5. **num(N)**：表示以节点 $N$ 为根的子树中叶节点的数量。
