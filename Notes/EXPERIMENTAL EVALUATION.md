...
- **Query Length**（查询长度）：表示查询的长度范围，例如 "1-11" 表示查询长度均匀分布在 1 到 11 之间（包含 1 和 11）。
- **Recovery Metrics**（恢复指标）：包括以下四个指标：
  1. **Unique Token**（唯一标记）：恢复的唯一标记的百分比。
  2. **Token with Repetition**（带重复的标记）：包括重复标记在内的恢复百分比。
  3. **Initial Path**（初始路径）：恢复的初始路径的百分比。
  4. **Query**（查询）：完整查询的恢复百分比。
...
1. **查询长度范围的影响**：
   - 查询长度范围越大（如 "1-11"），恢复率通常更高。
   - 当查询长度范围缩小（如 "3-13"），恢复率略有下降。

2. **各恢复指标的表现**：
   - **Unique Token**：恢复率从 **60.1%**（1-11）逐渐下降到 **51.8%**（3-13）。
   - **Token with Repetition**：恢复率最高，范围在 **81.9%**（1-11）到 **72.3%**（3-13）。
   - **Initial Path**：恢复率在 **66.3%**（1-11）到 **53.2%**（3-13）之间。
   - **Query**：完整查询的恢复率从 **63.6%**（1-11）下降到 **50.4%**（3-13）。

3. **查询长度对恢复率的影响**：
   - 当查询长度范围较大（如 1-11）时，恢复率更高，可能是因为短查询（如长度 1 或 2）更容易被恢复，且有助于优化较长查询的恢复。
   - 当查询长度范围较窄（如 3-13）时，恢复率略有下降，尤其是 **Unique Token** 和 **Query** 指标。

---

### 1.11 总结

- **恢复率随查询长度范围变化而变化**：较大的查询长度范围（如 1-11）能显著提高恢复率，尤其是短查询的加入有助于优化整体攻击效果。
- **带重复标记的恢复率最高**：这表明攻击对标记频率的利用效果较好。
- **完整查询恢复率较低**：尽管恢复率较高，但完整查询的恢复仍然是最具挑战性的环节。

这张表格清晰地展示了攻击在不同查询长度范围下的性能差异，为进一步优化攻击策略提供了重要参考。
