...
![image-20241231145417249](https://ceyewan.oss-cn-beijing.aliyuncs.com/typora/image-20241231145417249.png)
...
# 设置列名的中文显示
df_results.columns = ['隐层配置', '学习率', '准确率(%)', '训练时间(s)', '测试时间(s)']
...
![image-20241231145555044](https://ceyewan.oss-cn-beijing.aliyuncs.com/typora/image-20241231145555044.png)
...
def plot_nn_results(results):
    ...
    plt.ylabel('准确率', fontproperties=font)
    plt.title('神经网络: 学习率与准确率的关系', fontproperties=font)
    plt.legend()
    plt.grid(True)
    plt.xscale('log')
    plt.show()

plot_nn_results(mlp_results)
```
...
def numpy_nn_experiment(X_train, X_test, y_train, y_test, hidden_sizes, learning_rates):
    ...
    for hidden_size in hidden_sizes:
        for lr in learning_rates:
            print(f"正在测试 hidden_size={hidden_size}, learning_rate={lr}...")
...
![image-20241231145745404](https://ceyewan.oss-cn-beijing.aliyuncs.com/typora/image-20241231145745404.png)
