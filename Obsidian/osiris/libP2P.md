...
除了在开发 libp2p 应用程序时编写的协议外，libp2p 本身还定义了几种用于核心功能的基础协议。

#### 5.3.1 Common patterns

以下描述的协议均使用协议缓冲区（即 protobuf）来定义消息模式。

消息通过一种非常简单的约定在网络上交换，该约定在二进制消息有效载荷前加上一个表示有效载荷长度（以字节为单位）的整数。该长度被编码为 protobuf varint（可变长度整数）。
